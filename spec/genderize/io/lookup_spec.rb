...
RSpec.describe Genderize::Io::Lookup do
  ...
  describe '.initialize' do
    it 'returns {}' do
      expect(klass_1.data).to eq({})
    end
  end

  describe '.determine' do
    it 'returns true for count key' do
      expect(klass_2.data.key?('count')).to eq(true)
    end
  end
  ...
  describe '.url' do
    ...
    it "returns '#{base_url}'" do
      expect(klass_1.url).to eq(base_url)
    end
  ...
  Genderize::Io::Lookup::HEADER_KEYS.each do |key|
    describe ".#{key}" do
      it 'returns nil' do
        expect(klass_1.send(key)).to eq(nil)
      end

      it 'returns an integer' do
        expect(klass_2.send(key).is_a?(Integer)).to eq(true)
      end
    end
  end
  ...
  Genderize::Io::Lookup::RESPONSE_KEYS.each do |key|
    describe ".#{key}" do
      ...
      it "returns value in response_hash['#{key}']" do
        expect(klass_2.send(key)).to eq(response_hash[key])
      end
    end
  end
  ...
  describe '.param_name' do
    it 'returns "name=kim"' do
      expect(klass_1.send(:param_name)).to eq('name=kim')
    end
  end
